...
from pptx import Presentation # [수정] 오타 해결
import requests
...
# --- 1. 페이지 설정 및 데이터 초기화 ---
_ = st.set_page_config(page_title="GapFinder v2 (v21.6)", layout="wide")
...
def run_ai_analysis(data, step, insight="", brand_ctx="", consumer_raw=""):
    ...
    try:
        ...
        prompts = {
            "brand": f"{p_base}[Thesis] 자사 브랜드 분석. 강점 및 소비자 소구 언어 도출. 인사이트: {insight}",
            "comp": f"{p_base}[Competitor] 입력된 경쟁사만 분석. 자사({brand_ctx[:200]})와 대비하여 비어 있는 기회(White Space) 발굴.",
            "consumer": f"{p_base}[Antithesis] 네이버/구글/유튜브 통합 데이터 분석. 소비자의 실제 페인포인트 도출.",
